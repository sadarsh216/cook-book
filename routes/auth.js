    ...
    if(!_user) return res.status(400).send({msg:"User doesnot exists"})
        ...
        bcrypt.compare(req.body.password, _user.password).then(result =>{
            if(result){
                ...
                const token = jwt.sign(_responseData,process.env.JWT_SECRET_TOKEN);
                _responseData.token = token;
                res.header("auth-token", token).send(_responseData);
            }else {
                res.status(400).send({msg:"Wrong email id or password"});
            }
        })
    }
})
